...
## Bus Protocol
1. On accept: reply to whoever spawned you — `bus_post(channel="content", from="humanizer", to="<delegator from brief>", type="status", body="On it — <one line>")`. If no `delegator` (standalone run), use `to="*"`.
2. On a blocking question: `bus_post(channel="content", from="humanizer", to="<delegator>", type="question", body="<decision needed>")`, then wait for the `answer`.
3. On completion: `bus_post(channel="content", from="humanizer", to="<delegator>", type="done", body="<summary of work done>", ref="<output file path>")` (use `to="*"` only for a standalone run).
4. On error: `bus_post(channel="content", from="humanizer", to="<delegator>", type="alert", body="Error: <what failed>")`.
